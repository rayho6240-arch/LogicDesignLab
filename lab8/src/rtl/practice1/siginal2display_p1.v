    ...
    always @(*) begin
        ...
        for (row = 0; row < 8; row = row + 1) begin
            for (col = 0; col < 8; col = col + 1) begin
                display_map[row][col] =  map[(7-col)*8 + row]; //注意矩陣的syntax //把col 變成列(參考註解掉的原邏輯)//從最後一col 往回讀，逆時針轉了90度。// 順時針就是 map[col*8 + (7-row)] //逆時針map[(7-col)*8 + row]
            end
        end
        /*
        //直接讀取
